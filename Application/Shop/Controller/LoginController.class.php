...
class LoginController extends Controller {
    public function index(){
        ...
        if($result['status']==0){
            return show(0,'账号正在审核中...');
        }
        //$t=time();
        //print_r($t.'---'.$result['end_time']);exit;
        $time = time()-$result['end_time'];
        if($time>=0){
          return show(0,'账号已过期，请尽快联系总平台');
        }
        //echo md5($data['password']).'<br>'.$result['password'];exit;
        if(get_md5($data['password']) !== $result['password']){
            return show(0,'密码不正确');
        }
        session('shopUser',$result);  //session赋值
        return show(1,'登陆成功');
    	}
      $this->display('login');
    }
